...
public class SimpleTools extends Object {
	...
	public static String shorterDouble(double paraDouble, int paraLength) {
		...
		if ((paraDouble < 0.001) && (-0.001 < paraDouble) || (paraDouble < -9999) || (9999 < paraDouble))
		...
		for (int i = 0; i < secondArray.length; i++) {
			...
			if (!found)
				throw new Exception("Error occured in SimpleTool.stringMinusString(), \n" + "\t" + secondArray[i]
						+ "is not included in " + prmFirstString);
		} // Of for i
	...
	public static int[] parseIntArray(String paraString, int paraNumberOfInts, char paraSeparator) throws Exception {
		...
		try {
			for (int i = 0; i < paraNumberOfInts - 1; i++) {
				currentString = remainingString.substring(0, remainingString.indexOf(paraSeparator)).trim();
				returnArray[i] = Integer.parseInt(currentString);
				remainingString = remainingString.substring(remainingString.indexOf(paraSeparator) + 1).trim();
			} // Of for i
	...
	public static double[] parseDoubleArray(String paraString) throws Exception {
		...
		try {
			while (remainingString.indexOf(separator) > 0) {
				currentString = remainingString.substring(0, remainingString.indexOf(separator)).trim();
				tempArray[arrayLength] = Double.parseDouble(currentString);
				arrayLength++;
				if (arrayLength >= MAX_PARSE_ARRAY_LENGTH)
					throw new Exception("The array length should not exceed " + MAX_PARSE_ARRAY_LENGTH);
				remainingString = remainingString.substring(remainingString.indexOf(separator) + 1).trim();
			} // Of while
	...
	public static boolean[] stringToBooleanArray(String paraString, char paraSeparator) throws Exception {
		...
		for (int i = 0; i < tempLength; i++) {
			if (paraString.charAt(i * 2) == '0') {
				...
				throw new Exception("Error occurred in SimpleTool.stringToBooleanArray(String, char)."
						+ "\r\n Unsupported boolean value '" + paraString.charAt(i * 2) + "'. It should be 0 or 1.");
			}
		} // Of for i
		return resultArray;
	}// Of booleanArrayToString
	 ...
	 ********************************** 
	 */
	private static int valueBasedLongArrayPartition(long[] paraData, int[] paraMeasuredValues, int paraLeft,
			int paraRight) throws Exception {
	...
	public static boolean isSubset(boolean[] paraFirstSet, boolean[] paraSecondSet) throws Exception {
		if (paraFirstSet.length != paraFirstSet.length) {
			throw new Exception(
					"Error occurred in SimpleTool.isSubset(). Boolean arrays should" + " have the same length");
		} // Of if
	...
	public static int[][] binarySetFamilyMaximalIntersection(int[][] paraFirstSetFamily, int[][] paraSecondSetFamily) {
		...
		//judge the maximal intersection critical:
		//can modify improve the intersection,this method may find the max intersection
...
//	 * ************************
//	 * the setMinus 
//	 * @param paraIntersection
//	 * @param paraFirstArray
//	 * @param paraSecondArray
//	 * @return
//	 **************************
//	 */
//	public static int[] setMinus(int[] paraIntersection, int[] paraFirstArray, int[] paraSecondArray) {
//		int[] resultArray = new int[paraFirstArray.length];
//		int resultArrayIndex = 0;
//		for (int i = 0; i < paraIntersection.length; i++) {
//			for (int j = 0; j < paraFirstArray.length; j++) {
//				if (paraIntersection[i] == paraFirstArray[j]) {
//					continue;
//				}else {
//					resultArray[resultArrayIndex] = paraFirstArray[j];
//					resultArrayIndex++;
//					break;
//				}//of else
//			}//of for j
//			for (int j = 0; j < paraSecondArray.length; j++) {
//				if (paraIntersection[i] == paraSecondArray[j]) {
//					continue;
//				}else {
//					resultArray[resultArrayIndex] = paraSecondArray[j];
//					resultArrayIndex++;
//				}//of else
//			}//of for j
//		}//of for i
//		System.out.println("the resultArrayIndex is ：" + resultArrayIndex);
//		return resultArray;
//	}
	/**
	 ************************* 
	...
	public static int[] minusSets(int[] paraFirstArray, int[] paraSecondArray) {
		if (paraFirstArray.length > paraSecondArray.length) {
